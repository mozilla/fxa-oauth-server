...
const QUERY_CLIENT_UPDATE = 'UPDATE clients SET ' +
  ...
  'publicClient=COALESCE(?, publicClient) ' +
  'WHERE id=?';
const QUERY_CLIENT_DELETE = 'DELETE FROM clients WHERE id=?';
const QUERY_CODE_INSERT =
  'INSERT INTO codes (clientId, userId, email, scope, authAt, offline, code, code_challenge_method, code_challenge) ' +
  'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
MysqlStore.prototype = {
  ...
  generateCode: function generateCode(codeObj) {
    ...
    return this._write(QUERY_CODE_INSERT, [
      ...
      !!codeObj.offline,
      hash,
      codeObj.code_challenge_method,
      codeObj.code_challenge
    ]).then(function() {
      return code;
    });
  },
