...
const QUERY_CLIENT_REGISTER =
  ...
  'trusted, canGrant, publicClient, applicationScope) ' +
  'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);';
...
const QUERY_CLIENT_UPDATE = 'UPDATE clients SET ' +
  ...
  'applicationScope=COALESCE(?, applicationScope) ' +
  'WHERE id=?';
const QUERY_CLIENT_DELETE = 'DELETE FROM clients WHERE id=?';
const QUERY_CODE_INSERT =
  'INSERT INTO codes (clientId, userId, email, scope, authAt, offline, code, codeChallengeMethod, codeChallenge, derivedKeyBundle) ' +
  'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
const QUERY_APP_SCOPE_FIND =
  'SELECT * ' +
  'FROM applicationScopes ' +
  'WHERE applicationScopes.applicationScope=?;';
const QUERY_APPLICATION_SCOPES_INSERT =
  'INSERT INTO applicationScopes (applicationScope, salt, description) ' +
  'VALUES (?, ?, ?);';
...
MysqlStore.prototype = {
  ...
  registerClient: function registerClient(client) {
    ...
    return this._write(QUERY_CLIENT_REGISTER, [
      ...
      !! client.canGrant,
      !! client.publicClient,
      client.applicationScope ? client.applicationScope : null
    ]).then(function() {
  ...
  getApplicationScope: function getApplicationScope (appScope) {
    return this._readOne(QUERY_APP_SCOPE_FIND, [appScope]);
  },

  registerApplicationScope: function registerApplicationScope (appScope) {
    return this._write(QUERY_APPLICATION_SCOPES_INSERT, [
      appScope.applicationScope,
      buf(appScope.salt),
      appScope.description
    ]);
  },
