...
const QUERY_CLIENT_REGISTER =
  ...
  'VALUES (?, ?, ?, ?, ?, ?, ?);';
const QUERY_CLIENT_OWNER_INSERT =
  'INSERT INTO clientOwners ' +
  '(userId, clientId) ' +
  'VALUES (?, ?);';
...
MysqlStore.prototype = {
  ...
  registerClientOwner: function registerClientOwner(userId, clientId) {
    if (!userId || !clientId) {
      var err = new Error('Owner registration requires user and client id');
      return P.reject(err);
    }

    logger.debug('registerClientOwner: ', userId, clientId);
    return this._write(QUERY_CLIENT_OWNER_INSERT, [
      userId,
      clientId
    ]);
  },
