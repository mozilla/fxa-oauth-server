...
const QUERY_ACTIVE_CLIENT_TOKENS_BY_UID =
  'SELECT clients.name, clients.id, tokens.createdAt FROM clients, tokens ' +
  'WHERE tokens.expiresAt > NOW() AND clients.id = tokens.clientId AND tokens.userId=? ' +
  'LIMIT 100;';
...
MysqlStore.prototype = {
   ...
   * @returns {Promise}
   */
  getActiveClientTokensByUid: function getActiveServicesByEmail(uid) {
    if (! uid) {
