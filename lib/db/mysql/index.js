...
const QUERY_ACTIVE_CLIENT_TOKENS_BY_UID =
  'SELECT c.name, c.id, t.scope FROM fxa_oauth.clients AS c, fxa_oauth.tokens as t ' +
  'WHERE t.expiresAt > NOW() AND c.id = t.clientId AND uid=?';
const DELETE_ACTIVE_TOKENS_BY_CLIENT_AND_UID =
  'DELETE FROM tokens WHERE clientId=? AND uid=?';
...
MysqlStore.prototype = {
   ...
   * @returns {*}
   */
  getActiveServicesByEmail: function getActiveServicesByEmail(uid) {
    if (! uid) {
   ...
   * @returns {*}
   */
  deleteTokensByService: function deleteTokensByService(clientId, uid) {
    if (! clientId || ! uid) {
