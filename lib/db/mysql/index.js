...
// N.B., this sort order with limit is not actually fully deterministic
const QUERY_EXPIRED_TOKENS =
  'SELECT expiresAt, token, clientId FROM tokens WHERE expiresAt > ? AND expiresAt < NOW() ORDER BY expiresAt ASC, clientId, token LIMIT ?';
                                                          ...
                                                          deleteBatchSize = 200)
  {
    ...
    return P.all(clientIds)
      ...
      .then(() => {
        ...
        return self._write(QUERY_LAST_PURGE_TIME)
          .then((res) => {
            lastPurgeTime = (res && res[0] && res[0].value) || '2015-01-01 00:00:00';
      ...
      .then(() => {
        ...
        const promiseWhile = P.method(() => {
          ...
          return self._write(QUERY_EXPIRED_TOKENS, [lastPurgeTime, deleteBatchSize])
            .then((res) => {
              logger.info('purgeExpiredTokensById', { rowsReturned: res.length });

              const tokensForDeletion = res.filter((row) => {
                const expiresAt = moment.utc(row.expiresAt).format('YYYY-MM-DD HH:mm:ss');
