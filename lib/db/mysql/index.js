...
MysqlStore.prototype = {
  ...
  getPublicClients: function getPublicClients() {
    return this._read(QUERY_PUBLIC_CLIENTS_LIST);
  },
  ...
  revokePublicTokens: function revokePublicTokens(userId) {
    const uid = buf(userId);

    return this.getPublicClients().then((publicClients) => {
      const publicClientIds = publicClients.map(function(client) {
        return client.id;
      });

      return this._write(QUERY_DELETE_ACCESS_TOKEN_FOR_PUBLIC_CLIENTS, [uid, publicClientIds])
        .then(this._write.bind(this, QUERY_DELETE_REFRESH_TOKEN_FOR_PUBLIC_CLIENTS, [uid, publicClientIds]))
    });
  },
