...
function applicationScopes() {
  var appScopes = config.get('applicationScopes');
  if (appScopes && appScopes.length) {
    logger.debug('appScopes.loading', JSON.stringify(appScopes));

    return P.all(appScopes.map(function(appScope) {
      return exports.getApplicationScope(appScope.applicationScope).then(function(existing) {
        if (existing) {
          logger.verbose('appScopes.existing', appScope);
          return;
        }

        return exports.registerApplicationScope(appScope);
      });
    }));
  }
}
...
exports._initialClients = function() {
  return preClients().then(serviceClients).then(applicationScopes);
};
