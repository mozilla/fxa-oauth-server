...
/**
 * Some pre-defined applicationScopes that will be inserted into the DB
 */
function scopes() {
  ...
  if (scopes && scopes.length) {
    ...
    return P.all(scopes.map(function(scope) {
      return exports.getApplicationScope(scope.applicationScope).then(function(existing) {
        if (existing) {
          logger.verbose('scopes.existing', scope);
          return;
        }

        return exports.registerApplicationScope(scope);
      });
    }));
  }
}
