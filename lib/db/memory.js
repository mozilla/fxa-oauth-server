...
MemoryStore.prototype = {
  ...
  registerClientOwner: function registerClientOwner(userId, clientId) {
    var entry = {
      userId: userId,
      clientId: clientId,
      createdAt: new Date()
    ...
    logger.debug('registerClientOwner', entry);
    this.clientOwners[uniqueHexId] = entry;
    return P.resolve();
  },
  ...
  getClient: function getClient(id) {
    var hex = unbuf(id);
    var client = this.clients[hex];

    if (client) {
      Object.keys(this.clientOwners).map(function (id) {
        var owner = this.clientOwners[unbuf(id)];
        if (unbuf(owner.clientId) === hex) {
          client.owner = owner.userId;
        }
      }, this);
    }
