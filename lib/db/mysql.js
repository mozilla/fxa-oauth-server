...
const QUERY_CODE_INSERT =
  ...
  '(?, ?, ?, ?, ?)';
const QUERY_PUBKEY_INSERT =
  'INSERT INTO pubkeys (clientId, userId, email, scope, type, pubkey) VALUES ' +
  '(?, ?, ?, ?, ?, ?)';
const QUERY_PUBKEY_FIND = 'SELECT * FROM pubkeys WHERE pubkey=?';
...
MysqlStore.prototype = {
  ...
  storePubKey: function storePubKey(pubkey, code) {
    ...
    return this.removeCode(code.code).then(function() {
      var d = P.defer();
      conn.query(QUERY_PUBKEY_INSERT,
        [k.clientId, k.userId, k.email, k.scope.join(' '), k.type, pubkey],
        function(err) {
          if (err) {
            return d.reject(err);
          }
  ...
  getPubKey: function getPubKey(pubkey) {
    var d = P.defer();
    this._connection.query(QUERY_PUBKEY_FIND, [pubkey],
      function(err, rows) {
        if (err) {
          return d.reject(err);
        }
