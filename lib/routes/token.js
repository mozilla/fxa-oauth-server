...
const PAYLOAD_SCHEMA = Joi.object({
  ...
  code_verifier: validators.codeVerifier // PKCE code_verifier
    .when('grant_type', {
      is: GRANT_JWT,
      then: Joi.forbidden()
    }),
...
function confirmPkceCode(id, code, pkceVerifier) {
  return db.getCode(buf(code)).then(function(codeObj) {
    ...
    if (pkceHash(pkceVerifier) === codeObj.code_challenge) {
...
function confirmPkceClient(id) {
  return db.getClient(buf(id)).then(function(client) {
    ...
    if (!client.publicClient) {
      ...
      throw AppError.unknownClient(id); // TODO: custom error?
    }
