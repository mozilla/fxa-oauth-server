...
const PAYLOAD_SCHEMA = Joi.object({
  ...
  existing_grant: validators.token
    .optional()
...
module.exports = {
  ...
  handler: function tokenEndpoint(req, reply) {
    ...
    .then(function(vals) {
      ...
      if (params.existing_grant) {
        return db.getRefreshToken(encrypt.hash(params.existing_grant)).then(function (tok) {
          if (! tok) {
            throw new AppError.unauthorized('Unknown existing grant');
          }
          if (tok.userId.toString('hex') !== vals.userId.toString('hex') ||
              tok.clientId.toString('hex') !== vals.clientId.toString('hex')) {
            throw new AppError.unauthorized('Invalid grant');
          }
          vals.scope = Scope(vals.scope).merge(Scope(tok.scope)).values();
          return vals;
        });
      } else if (vals.includeGrantedScopes) {
        return db.getActiveClientsByUid(vals.userId.toString('hex')).then(function (tokens) {
          const allScopes = []; // Contain duplicates, but Scope#merge drops them.
          for (const token of tokens) {
            allScopes.push(...token.scope);
          }
          vals.scope = Scope(vals.scope).merge(Scope(allScopes)).values();
          return vals;
        });
      }
      return vals;
