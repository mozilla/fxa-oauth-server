  ...
  handler: function tokenEndpoint(req, reply) {
    ...
    P.try(function() {
      if (params.grant_type === GRANT_AUTHORIZATION_CODE) {
        ...
        return db.getClient(buf(clientId)).then(function(dbClient) {
          if (! dbClient) {
          ...
          if (dbClient.publicClient) {
            if (params.client_secret) {
              throw new AppError.invalidRequestParameter('client_secret');
            }

            confirmClientPromise = confirmPkceClient(params.client_id);
          } else {
            confirmClientPromise = confirmClient(params.client_id, params.client_secret);
          }
