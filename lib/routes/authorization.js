...
function generateCode(claims, client, scope, req) {
  // TODO: pending codes <-> to access token
...
module.exports = {
  ...
  handler: function authorizationEndpoint(req, reply) {
    ...
    .then(function (result) {
      if(req.payload.derivedKeyBundle) {
        return db.setDerivedKey(uid + req.payload.client_id,
          req.payload.derivedKeyBundle).then(function () {
          return result;
        })
      }
