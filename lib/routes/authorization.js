...
function generateCode(claims, client, scope, req) {
  return db.generateCode({
    ...
    derivedKeyBundle: req.payload.derivedKeyBundle,
  }).then(function(code) {
    ...
    P.all([
      ...
      db.getClient(Buffer(req.payload.client_id, 'hex')).then(function(client) {
        ...
        if (! client) {
          ...
          // 3 checks?
            // 1. Notes only scopes: is this client allowed these applicationScopes?
            // 2. Non-Notes asking for notes: Notes asking for non-Notes scope? (This client id does not allow 'sync scopes'
            // 3. Notes not asking for Notes?

          // does this client support derivedKeyBundle
        }
