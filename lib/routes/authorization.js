...
function generateCode(claims, client, scope, req) {
  if (client.publicClient && req.payload.include_granted_scopes) {
    // From RFC (https://tools.ietf.org/html/draft-ietf-oauth-incremental-authz-00):
    // the "include_granted_scopes" feature MUST NOT be enabled for such public
    // client requests.
    throw AppError.notConfidentialClient();
  }
  return db.generateCode({
    ...
    includeGrantedScopes: req.payload.include_granted_scopes,
  }).then(function(code) {
...
module.exports = {
  validate: {
    payload: {
      ...
      include_granted_scopes: Joi.boolean().optional(),
