...
Scope.prototype = {
  ...
  merge: function merge(scope) {
    if (! (scope instanceof Scope)) {
      throw new Error('Scope merge only works with Scope instances.');
    }
    const ours = this.values();
    const theirs = scope.values();
    const merged = Array.from(new Set([...ours, ...theirs]));
    return Scope(merged);
  },
